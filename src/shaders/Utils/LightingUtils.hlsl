...
// ambient lighting
float3 ComputeAmbient(Material mat, float3 pos, float3 normal, float3 toEye)
{
    float3 F0 = DIELECTRIC_SPECULAR;
    float3 lambertianDiffuse = mat.DiffuseAlbedo.xyz / PI;
...
float3 ComputeIBL(Material mat, float3 pos, float3 normal, float3 toEye)
{
    float3 F0 = DIELECTRIC_SPECULAR;
    float3 albedo = mat.DiffuseAlbedo.xyz;
    // gltf specs
                       ...
                       float3 shadowFactor)
{
    ...
    float3 IBL = ComputeIBL(mat, pos, normal, toEye);
    float3 ambient = ComputeAmbient(mat, pos, normal, toEye);

    // Full lighting 
    return float4(IBL * mat.AO + ambient + result, 1.0);
}
